...
void Shader::SetUniform1i(const std::string& uName, const int val)
{
	glUniform1i(glGetUniformLocation(this->ID, uName.c_str()), val);
}

void Shader::SetUniformMat4(const std::string& uName, glm::mat4& mat)
{
	glUniformMatrix4fv(GetUniformLocation(uName), 1, GL_FALSE, glm::value_ptr(mat));
}

uint32_t Shader::GetUniformLocation(const std::string& uName)
{ 
	return glGetUniformLocation(this->ID, uName.c_str());
}